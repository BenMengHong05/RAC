                ...
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0 " id="search-table">
                        ...
                        @if ($innovations->isEmpty())
                            <tr>
                                <td colspan="5" class="text-center">No innovations found.</td>
                            </tr>
                        @else
                            @foreach ($innovations as $index => $innovation)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td><img src="{{ asset('images/' . $innovation->image) }}" alt=""
...
<script>
    function confirmDelete(innovationId) {
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('delete-form-' + innovationId).submit();
                Swal.fire({
                    title: "Deleted!",
                    text: "The innovation has been deleted.",
                    icon: "success"
                });
            }
        });
