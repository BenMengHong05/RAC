...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="mb-4">
            <h2 class="m-0 p-0  text-muted" >{{__('messages.Contents')}} / {{__('messages.Posts')}} / <span class="text-dark">{{__('messages.Alumins')}}</span></h2>
        </div>
        <div class="card">
        ...
        @endif
            <div class="car-header d-flex mt-3 align-items-center justify-content-between" style="min-height: 7vh">
                <div class="add d-flex align-items-center gap-4 ms-4 " style="width: 60%">
                    <form action="{{ route('alumni_search') }}" method="GET" class="d-flex">
                ...
                <form action="{{ route('alumni_create') }}" class="ps-2 " method="GET">
                    @csrf
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0" id="search-table">
                        ...
                        @if ($alumnis->isEmpty())
                            <tr>
                                <td colspan="5" class="text-center">No alumnis found.</td>
                            </tr>
                        @else
                            @foreach ($alumnis as $index => $alumni)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td><img src="{{ asset('images/' . $alumni->image) }}" alt=""
    ...
    function confirmDelete(alumniId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('delete-form-' + alumniId).submit();
                Swal.fire({
                    title: "Deleted!",
                    text: "The alumni has been deleted.",
                    icon: "success"
                });
            }
        });
