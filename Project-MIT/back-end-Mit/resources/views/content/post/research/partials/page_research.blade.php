...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="mb-4">
            <h2 class="m-0 p-0 text-muted">{{__('messages.Contents')}} / {{__('messages.Posts')}} / <span class="text-dark">{{__('messages.Researchs')}}</span></h2>
        </div>
        <div class="card">
        ...
        @endif
            <div class="car-header d-flex align-items-center justify-content-between" style="min-height: 7vh">
                <div class="add d-flex align-items-center gap-4 ms-4 " style="width: 60%">
                    <form action="{{ route('research_create') }}" method="GET" class="d-flex">
                ...
                <form action="{{ route('research_create') }}" class="ps-2 " method="GET">
                    @csrf
                ...
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0 " id="search-table">
                        ...
                        @if ($researchs->isEmpty())
                            <tr>
                                <td colspan="5" class="text-center">No researchs found.</td>
                            </tr>
                        @else
                            @foreach ($researchs as $index => $research)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td><img src="{{ asset('images/' . $research->image) }}" alt=""
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item"
                                                    href="{{ route('research_show', $research->id) }}"><i
                                                        class="bx bx-duplicate me-1"></i>{{__('messages.Detail')}}</a>
                                                <a class="dropdown-item"
                                                    href="{{ route('research_edit', $research->id) }}"><i
                                                        class="bx bx-edit-alt me-1"></i>{{__('messages.Edit')}}</a>
                                                <form id="delete-form-{{ $research->id }}"
                                                    action="{{ route('research_delete', ['id' => $research->id]) }}"
                                                    method="POST" style="display: inline;">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" class="dropdown-item text-danger"
                                                        onclick="confirmDelete({{ $research->id }})">
