<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="mb-4">
            <h2 class="m-0 p-0  text-muted">{{__('messages.Contents')}} / <span class="text-dark">{{__('messages.categories_image')}}</span></h2>
        </div>
        <div class="card">
            ...
            <div class="table-responsive "> {{-- style="overflow: auto;white-space: nowrap;" --}}
                <table class="table" id="search">
                    ...
                    <tbody class="table-border-bottom-0" id="search-table">
                        @if ($categories_images->isEmpty())
                            <tr>
                                <td colspan="5" class="text-center">No categories_images found.</td>
                            </tr>
                        @else
                            @foreach ($categories_images as $index => $categorie_image)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                        <td><img src="{{ asset('images/' . $categorie_image->image) }}" alt=""
                                                style="width: 10vh;height: 10vh"></td>
                                    <td class="categorie-name">{{ $categorie_image->categorie->name }}</td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                ...
                                                <form id="delete-form-{{ $categorie_image->id }}"
                                                    ...
                                                    <button type="button" class="dropdown-item text-danger"
                                                        onclick="confirmDelete({{ $categorie_image->id }})">
...
<script>
    function confirmDelete(categorie_imageId) {
        Swal.fire({
            title: "Are you sure?",
            ...
            if (result.isConfirmed) {
                document.getElementById('delete-form-' + categorie_imageId).submit();
                Swal.fire({
                    title: "Deleted!",
                    text: "The categorie_image has been deleted.",
                    icon: "success"
                });
            }
        });
